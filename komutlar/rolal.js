...
exports.run = async (bot, message, args) => {
 ...
 if (!message.member.roles.cache.get(yetkili) & !message.member.hasPermission("ADMINISTRATOR")) return message.react(basarisiz);
  ...
  let rMember = message.guild.member(message.mentions.users.first()) || message.guild.members.cache.get(args[0]);
    if (!rMember) return message.channel.send(new Discord.MessageEmbed().setDescription(`${basarisiz} Rol almam için bir kişiyi etiketlemelisin!`)).then(x => x.delete({timeout: 3000}));
    let role = message.mentions.roles.first();

    if (!role) return message.channel.send(new Discord.MessageEmbed().setColor('RED').setDescription(`${basarisiz} Rol alabilmem için rolü belirtmelisiniz!`)).then(x => x.delete({timeout: 3000}));
    ...
    if (!rMember.roles.cache.has(aRole.id)) return message.channel.send(new Discord.MessageEmbed().setColor('RED').setDescription('<
                                                                                                                                  
                                                                                            a:plantacarp:815252488168931368> Bu rolü bu kullanıcıda görüyorum!`)).then(x => x.delete({timeout: 3000}));  
