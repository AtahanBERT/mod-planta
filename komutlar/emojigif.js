...
if(!args[0]) return message.channel.send(new MessageEmbed().setDescription(`${basarisiz} ${message.author}, Bir link ve ad yazmalısın. Örnek: **.eegif https://cdn.discordapp.com/emojis/601379275769118731.gif plantateam**`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000}));
...
if(!args[1]) return message.channel.send(new MessageEmbed().setDescription(`${basarisiz} ${message.author}, Bir emoji adı yazmalısın. Örnek: **.eegif https://cdn.discordapp.com/emojis/601379275769118731.gif plantateam**`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000}));
