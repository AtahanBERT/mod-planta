...
exports.run = async (bot, message, args) => {
 ...
 if (!message.member.roles.cache.get(yetkili) & !message.member.hasPermission("ADMINISTRATOR"));
    ...
    if (message.member.roles.highest.comparePositionTo(role) < 1) return message.channel.send(new Discord.MessageEmbed().setDescription(`${basarisiz} Vericeğin rol senden daha üstün!`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000}));
