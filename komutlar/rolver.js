...
exports.run = async (bot, message, args) => {
 ...
 if (!message.member.roles.cache.get(yetkili) & !message.member.hasPermission("ADMINISTRATOR")) return message.react(basarisiz);
  ...
  let rMember = message.guild.member(message.mentions.users.first()) || message.guild.members.cache.get(args[0]);
    if (!rMember) return message.channel.send(new Discord.MessageEmbed().setDescription(`<a:qmi2:809010861162233857> Rol vermem için bir kişiyi etiketlemelisin!`)).then(x => x.delete({timeout: 3000}));
    let role = message.mentions.roles.first()

    if (!role) return message.channel.send(new Discord.MessageEmbed().setColor('RED').setDescription(`<a:qmi2:809010861162233857> Rol verebilmem için rolü belirtmelisiniz!`)).then(x => x.delete({timeout: 3000}));
