...
app.get("/", (request, response) => {
  response.send(
    `Bot Aktif | Discord: https://discord.gg/yF6RRnq9Ks | İletisim Veya Uptime Etmek İçin Discordumuza Gelebilirsiniz.`
  );
...
client.on("message", async (msg, member, guild) => {
  {
    if (msg.content.toLowerCase() === "10s") {
      if (
        !msg.member.roles.cache.get(ayarlar.rewardsyetkili) &
    ...
    try {
      if (
        ...
        await client.channels.cache
          ...
          .send(
            new Discord.MessageEmbed()
              .setColor("GREEN")
              .setDescription(
                `${newUser} adlı kullanıcı (\`${tag}\`) tagımızı alarak bizi mutlu etti. Tagımızı aldığın için sana <@&${rol}> adlı rolü verdim. Ailemize Hoşgeldin :)`
              )
          );
        ...
        await client.guilds.cache.get(sunucu).members.cache.get(newUser.id)(
          client.guilds.cache.get(sunucu).members.cache.get(newUser.id).nickname
            .slice
        );
        await client.guilds.cache.get(sunucu).members.cache.get(newUser.id)(
          client.guilds.cache.get(sunucu).members.cache.get(newUser.id)
            .displayName
        );
      } //Planta Team
      if (
        ...
        await client.channels.cache
          ...
          .send(
            new Discord.MessageEmbed()
              .setColor("RED")
              .setDescription(
                `${newUser} adlı kullanıcı (\`${tag}\`) tagımızı çıkararak bizi üzdü. Tagımızı çıkardığın için senden <@&${rol}> adlı rolü aldım. Ailemize tekrardan bekleriz...`
              )
          );
        await client.guilds.cache
          .get(sunucu)
          .members.cache.get(newUser.id)
          .roles.remove(rol); //Planta Team
        await client.guilds.cache.get(sunucu).members.cache.get(newUser.id)(
          client.guilds.cache.get(sunucu).members.cache.get(newUser.id).nickname
            .slice
        );
...
client.on("message", async message => {
  ...
  if (reklamkick == "Açık") {
    const reklam = ["discord.app", "discord.gg", ".gg", "gg/", ".gg/"];
    if (reklam.some(word => message.content.toLowerCase().includes(word))) {
      if (message.author.id !== "429357746002067493")
        if (
          !message.member.roles.cache.get("832564510547378199") &
          ...
          if (uyarisayisi === null) {
            let ikrudka = new Discord.MessageEmbed().setDescription(
              ` <@${message.author.id}> Sunucuda reklam yapmak yasaktır!`
            );
          ...
          if (uyarisayisi === 2) {
            ...
            let ikrudka = new Discord.MessageEmbed()
              ...
              .setColor("#0054ff")
              .setDescription(
                ` <@${message.author.id}> Uyarılmasına Rağmen \`3\` Kez Reklam Yaptığı İçin Sunucudan Atıldı!`
              );
            return message.channel
              .send(ikrudka)
              .then(x => x.delete({ timeout: 3000 }));
...
client.on("guildBanAdd", async (guild, user) => {
  ...
  if (modlogs) {
    let embed = new Discord.MessageEmbed()
      .setColor("#080000")
      .setDescription(
        `\`${user.tag}\` adlı kullanıcı sunucudan yasaklandı. Yasaklayan kişi <@${yetkili.id}> `
      )
      .setFooter(`${botadi}`)
      .setTimestamp();
...
client.on("guildBanRemove", async (guild, user) => {
  let entry = await guild
    .fetchAuditLogs({ type: "BAN_REMOVE" })
    .then(audit => audit.entries.first());
  let yetkili = client.users.cache.get(entry.executor.id);
  let modlogs = kanal;
  const modlogkanal = guild.channels.cache.find(kanal => kanal.id === modlogs);
  ...
  if (modlogs) {
    let embed = new Discord.MessageEmbed()
      .setColor("#080000")
      .setDescription(
        `\`${user.tag}\` adlı kullanıcının yasaklaması kaldırıldı. Yasaklamayı kaldıran kişi <@${yetkili.id}> `
      )
      .setFooter(`${botadi}`)
      .setTimestamp();
    modlogkanal.send(embed);
  ...
  if (modlogs) {
    if (channel.type === "text") {
      let embed = new Discord.MessageEmbed()
        .setColor("#fffa00")
        .setDescription(
          `\`${channel.name}\` adlı metin kanalı oluşturuldu. Kanalı oluşturan kişi <@${user.id}>`
        )
    ...
    if (channel.type === "voice") {
      let embed = new Discord.MessageEmbed()
        .setColor("#fffa00")
        .setDescription(
          `\`${channel.name}\` adlı ses kanalı oluşturuldu. Kanalı oluşturan kişi <@${user.id}>`
        )
        .setFooter(`${botadi} `)
        .setTimestamp();
      modlogkanal.send(embed);
  ...
  if (modlogs) {
    if (channel.type === "text") {
      let embed = new Discord.MessageEmbed()
        .setColor("#fffa00")
        .setDescription(
          `\`${channel.name}\` adlı metin kanalı silindi. Kanalı silen kişi <@${user.id}>`
        )
        .setFooter(`${botadi} `)
        .setTimestamp();
      modlogkanal.send(embed);
    ...
    if (channel.type === "voice") {
      let embed = new Discord.MessageEmbed()
        .setColor("#fffa00")
        .setDescription(
          `\`${channel.name}\` adlı ses kanalı silindi. Kanalı silen kişi <@${user.id}>`
        )
        .setFooter(`${botadi} `)
        .setTimestamp();
      modlogkanal.send(embed);
...
client.on("roleDelete", async role => {
  ...
  if (modlogs) {
    let embed = new Discord.MessageEmbed()
      .setColor("#fffa00")
      .setDescription(
        `\`${role.name}\` adlı rol silinmiştir. Rolü silen kişi <@${user.id}>`
      )
      .setFooter(`${botadi} `)
      .setTimestamp();
    modlogkanal.send(embed);
...
client.on("emojiDelete", async emoji => {
  ...
  if (modlogs) {
    let embed = new Discord.MessageEmbed()
      .setColor("#fffa00")
      .setDescription(
        `\`${emoji.name}\` adlı emoji silinmiştir. Emojiyi silen kişi <@${user.id}>`
      )
      .setFooter(`${botadi} `)
      .setTimestamp();
    modlogkanal.send(embed);
...
client.on("roleCreate", async role => {
  ...
  if (modlogs) {
    let embed = new Discord.MessageEmbed()
      .setColor("#fffa00")
      .setDescription(
        `\`${role.name}\` adlı rol oluşturuldu. Rolü oluşturan kişi <@${user.id}>`
      )
      .setFooter(`${botadi} `)
      .setTimestamp();
...
client.on("emojiCreate", async emoji => {
  ...
  if (modlogs) {
    let embed = new Discord.MessageEmbed()
      .setColor("#fffa00")
      .setDescription(
        `\`${emoji.name}\` adlı emoji oluşturuldu. Emojiyi oluşturan kişi <@${user.id}>`
      )
      .setFooter(`${botadi} `)
      .setTimestamp();
    modlogkanal.send(embed);
...
client.on("messageUpdate", async (oldMessage, newMessage) => {
  ...
  let embed = new Discord.MessageEmbed()
    .setColor("#fffa00")
    .setDescription(
      `\`${newMessage.channel.name}\` adlı kanalda mesaj değiştirilmiştir. Mesajı değiştiren kişi <@${newMessage.author.id}>`
    )
...
client.on("messageDelete", async deletedMessage => {
  ...
  let embed = new Discord.MessageEmbed()
    .setColor("#fffa00")
    .setDescription(
      `\`${deletedMessage.channel.name}\` adlı kanalda mesaj silindi. Mesajın sahibi <@${deletedMessage.author.id}>`
    )
    .addField(
      "Silinen Mesaj:",
...
client.on("userUpdate", async user => {
  ...
  let channel = client.guilds.cache
    .get(sunucuid)
    .channels.cache.get(ayarlar.jailkanal); //nereye mesaj atmasını istiyorsanız, o kanalın tam ismini yazıniz!
  if (!tag) return;
  if (!cezalırol) return;
  if (!channel) return;
    ...
    if (member.user.username.includes(tag)) {
      ...
      setTimeout(function() {
        const tagalma = new Discord.MessageEmbed()
          ...
          .setColor("#e6f3b4")
          .setDescription(
            `<@${user.id}> adlı kişi, ${tag} tagını aldığından dolayı tüm yetkilerini alıp cezalı taga attım eğer tagını silerse yeniden kayıt olup aramıza katılabilir`
          )
      ...
      setTimeout(function() {
        user.send(
          `${tag}'ı aldığından dolayı malesef ki cezalıya atmak zorunluğunda kaldım tagını kaldırarak aramıza tekrar katılabilirsin 😊`
        );
      }, 3000);
    }
  } else {
    if (!member.user.username.includes(tag)) {
      ...
      setTimeout(function() {
        const tagsilme = new Discord.MessageEmbed()
          ...
          .setTitle(`Ceza Mahkemesi`)
          .setDescription(
            `<@${user.id}> adlı kişi, ${tag} tagını sildiğinden dolayı affedildi ve tekrardan kayıtsıza atıldı.`
          )
          .setFooter(member.user.username, member.user.avatarURL());
      ...
      setTimeout(function() {
        user.send(
          `${tag} ı Aldığın tagı geri çıkardığın için affedildin aramıza tekrardan hoşgeldin. 😊`
        );
      }, 3000);
    }
  }
...
client.on("guildMemberAdd", member => {
  ...
  if (member.user.username.includes(tag)) {
    ...
    member.send(
      `Görünüşe bakılırsa ${tag} adlı tagda bulunuyorsun aramıza katılmak için tagı kaldırabilirsin`
    );
  }
});
...
client.on("message", async msg => {
  ...
  if (msg.author.id === kisi) {
    msg.channel.send(
      new Discord.MessageEmbed()
        .setColor("BLACK")
        .setDescription(`${basari} <@${kisi}> Başarıyla Afk Modundan Çıktınız`)
    );
